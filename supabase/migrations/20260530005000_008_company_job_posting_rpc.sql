/*
  # Company job posting RPC

  Adds a server-side job posting function for company admins. The function
  checks the authenticated user's profile role and performs the insert as a
  security definer, so the app does not depend on a broad table INSERT policy.
*/
