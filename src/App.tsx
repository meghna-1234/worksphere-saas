    ...
    switch (currentPage) {
      ...
      case 'payments':
        if (profile?.role === 'company_admin') {
          return <CompanyModule page="payments" />;
        }
        return <StudentProfile activePage="payments" />;

      case 'jobs':
        if (profile?.role === 'company_admin') {
          return <CompanyModule page="jobs" />;
        }
        return <StudentProfile activePage="jobs" />;

      case 'dashboard':
        if (profile?.role === 'student') {
          return <StudentProfile activePage="dashboard" />;
        }
        if (profile?.role === 'employee') {
          return <EmployeeDashboard initialView="dashboard" />;
        }
        if (profile?.role === 'company_admin') {
          return <CompanyModule page="dashboard" />;
        }
        return <HRDashboard activePage={currentPage as any} />;
      case 'employees':
        if (profile?.role === 'company_admin') {
          return <CompanyModule page="employees" />;
        }
        return <HRDashboard activePage={currentPage as any} />;
      case 'analytics':
        if (profile?.role === 'company_admin') {
      ...
      case 'performance':
        return <EmployeeDashboard initialView="performance" />;
      case 'messages':
        return <ChatModule />;
      case 'meetings':
            ...
            <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center">
              <svg className="w-4 h-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
                  ...
                  strokeWidth={2}
                  d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 01-2-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"
                />
              </svg>
            </div>
            Photo
